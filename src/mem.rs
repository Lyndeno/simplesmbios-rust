...
type MemoryResult<T> = Result<T, MemError>;
...
impl MemDevice {
    pub fn from_source(source: SMBiosSource) -> MemoryResult<Option<Vec<Self>>> {
        match source {
    ...
    fn from_smbios_local_device() -> MemoryResult<Option<Vec<Self>>> {
        let data = table_load_from_device()?;
        Self::from_smbios_data(data)
    }

    fn from_smbios_dump(path: &Path) -> MemoryResult<Option<Vec<Self>>> {
        let data = load_smbios_data_from_file(path)?;
        Self::from_smbios_data(data)
    }

    fn from_smbios_data(data: SMBiosData) -> MemoryResult<Option<Vec<Self>>> {
